######################################################################################################
# THIS FILE IS FOR RUNNING THE PLATFORM GENERATOR AGENT AND PLAYER AGENT TOGETHER (ADVERSE SYSTEM).
# THE PLATFORMS ARE RANDOM EVERY LEVEL IN THIS GAME LOOP.
# THE PLAYER AGENT LEARNS TO TRAVERSE PLATFORMS.
...
#Rewards
def reward_from_env(curr_score,score_data,time_data, game_end_state):
    alpha = 0.5
...
while run:
    ...
    if game_over == False:
        ...
        move = np.random.choice(action_player)
        ...
        if len(platform_group) < MAX_PLATFORMS:
            p_w = np.random.randint(max(0.2,50*aux_w),max(0.2,100*aux_w)) #MODEL LEARNING
            ...
            """if random.SystemRandom().random() >0.5:
                ...
                if platform.rect.x + dell > SCREEN_WIDTH - p_w:
                    ...
                    p_x = platform.rect.x + dell
            """
            p_y = platform.rect.y - np.random.randint(min(149,50/(aux_y)), 150) #MODEL LEARNING
            platform = Platform(p_x,p_y,p_w)
            platform_group.add(platform)
            
            
            
            player_current_state = get_state()
            
        if np.random.random() <= agentPlayer.EPSILON:
            p_action = np.random.randint(0, 2)
        else:
            #action = np.argmax(agentPlayer.get_qs(current_state),axis=1)[0]
            #print(agentPlayer.get_qs(current_state).shape)
            p_action = np.argmax(agentPlayer.get_qs(player_current_state))
        pygame.event.post(action_player[p_action])
        ...
        p_reward = reward_player(player.max_height,prev_max_height,game_end_state)
        ...
        if player.rect.top > SCREEN_HEIGHT:
            ...
            agentPlayer.update_replay_memory((player_current_state,p_action,p_reward,player_state_next,True))
            ...
            time_taken = 0
            print("Level time taken: "+str(episode_time[-1]))
        agentPlayer.update_replay_memory((player_current_state,p_action,p_reward,player_state_next,False))
